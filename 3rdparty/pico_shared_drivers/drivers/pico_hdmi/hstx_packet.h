...
int hstx_packet_set_audio_samples(hstx_packet_t *packet, const audio_sample_t *samples, int num_samples,
                                  int frame_count);
// Same as hstx_packet_set_audio_samples but writes IEC 60958 channel status
// bits (44.1 kHz sample-frequency code + "consumer LPCM, copy permitted"
// mode) into the L/R VUCP bytes over the 192-frame block. Strict HDMI
// receivers (many display monitors) mute streams whose channel status is
// all-zero — "sample frequency not indicated" — even when Audio InfoFrame is
// present. Ported verbatim from https://github.com/.../pico_hdmi (the
// upstream that this vendored copy predates).
