#!/bin/sh
# HW_CONFIG 2 (Pico 2 + Adafruit DVI Breakout), doom_tiny_full built for the
# pico-bootLoader: image relinked into the 1.5 MB app slot at 0x10080000
# (DOOM_APP_SIZE=0x180000, 4 MB flash map). No WAD in flash — at boot the game
# copies /roms/doom/doom.whd from the SD card into PSRAM (src/pico/whd_sdload.c).
...
# Artifact: build_bl_full_adafruitdvisd/src/doom_tiny_full.uf2
cd "$(dirname "$0")" || exit 1
TAG=adafruitdvisd
. ./pico-env.sh
